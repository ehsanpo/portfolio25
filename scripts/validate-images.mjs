    ...
    folders.forEach((folder) => {
      ...
      markdownFiles.forEach((markdownFile) => {
        ...
        try {
          ...
          errors.push(
            `Error reading ${dirPath}/${folder}/${markdownFile}: ${error.message}`
          );
        }
      });
      ...
      if (imageFiles.length > 0) {
        ...
        imageFiles.forEach((imageFile) => {
          ...
          if (!isReferenced) {
            warnings.push(
              `Orphaned image (not referenced): ${dirPath}/${folder}/${imageFile}`
            );
          }
        });
      }
...
function validatePublicImages() {
  const publicImageDirs = ["public/images", "public/icons", "public/optimized"];
    ...
    if (fs.existsSync(fullDirPath)) {
      ...
      files.forEach((filePath) => {
        ...
        if (sizeInMB > 2) {
          ...
          warnings.push(
            `Large image file (${sizeInMB.toFixed(1)}MB): ${path.relative(
              rootDir,
              filePath
            )}`
          );
...
function checkImageOptimization() {
  ...
  if (!fs.existsSync(manifestPath)) {
    warnings.push(
      'Image optimization manifest not found. Run "npm run build:images"'
    );
    return;
  }
