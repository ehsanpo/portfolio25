  ...
  ["sv", "fa"].forEach((locale) => {
    ...
    if (!fs.existsSync(localeFile)) {
      ...
      try {
        ...
        errors.push(
          `Invalid JSON in src/data/i18n/${locale}.json: ${error.message}`
        );
      }
    }
  });
...
function calculateCoverage() {
  const coverage = {
    sv:
      stats.total > 0
        ? ((stats.translated.sv / stats.total) * 100).toFixed(1)
        : 0,
    fa:
      stats.total > 0
        ? ((stats.translated.fa / stats.total) * 100).toFixed(1)
        : 0,
  };
    ...
    folders.forEach((folder) => {
      ...
      ["sv", "fa"].forEach((locale) => {
        ...
        if (fs.existsSync(translationFile)) {
          const translationLength = fs.readFileSync(
            translationFile,
            "utf8"
          ).length;
          ...
          if (translationLength < originalLength * 0.5) {
            warnings.push(
              `${dirPath}/${folder}/${folder}.${locale}.md appears to be significantly shorter than original`
            );
          }
        }
      });
...
console.log(
  `   🇸🇪 Swedish coverage: ${coverage.sv}% (${stats.translated.sv}/${stats.total})`
);
console.log(
  `   🇮🇷 Persian coverage: ${coverage.fa}% (${stats.translated.fa}/${stats.total})`
);
...
if (
  parseFloat(coverage.sv) < minCoverage ||
  parseFloat(coverage.fa) < minCoverage
) {
