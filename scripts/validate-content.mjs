...
function validateFrontmatter(filePath, frontmatter, type) {
  ...
  if (
    frontmatter.status &&
    !["published", "draft", "archived"].includes(frontmatter.status)
  ) {
    warnings.push(
      `${filePath}: Invalid status '${frontmatter.status}', should be: published, draft, or archived`
    );
  }
  ...
  contentDirs.forEach((dirPath) => {
    ...
    folders.forEach((folder) => {
      ...
      if (!fs.existsSync(mainFilePath)) {
        errors.push(
          `Missing main markdown file: ${path.join(dirPath, folder, mainFile)}`
        );
        return;
      }
      ...
      try {
        ...
        validateFrontmatter(
          path.join(dirPath, folder, mainFile),
          frontmatter,
          type
        );
      } catch (error) {
        errors.push(
          `Error parsing ${path.join(dirPath, folder, mainFile)}: ${
            error.message
          }`
        );
      }
      ...
      locales.forEach((locale) => {
        ...
        if (fs.existsSync(translationPath)) {
          try {
            ...
            const { data: frontmatter } = matter(content);
            validateFrontmatter(
              path.join(dirPath, folder, translationFile),
              frontmatter,
              type
            );
          } catch (error) {
            errors.push(
              `Error parsing ${path.join(dirPath, folder, translationFile)}: ${
                error.message
              }`
            );
          }
