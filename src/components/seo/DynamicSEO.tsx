...
export function DynamicSEO({ content, type }: DynamicSEOProps) {
  useEffect(() => {
    ...
    const description = content.meta.description || content.meta.excerpt || `${content.meta.title} - Portfolio case study by Ehsan Pourhadi`;
    ...
    // Cleanup function
    return () => {
      const scriptToRemove = document.querySelector('script[data-portfolio-ld]');
      if (scriptToRemove) {
