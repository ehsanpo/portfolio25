...
export function TechStackVisualizer({
  ...
  const filteredSkills = skills.filter((skill) => {
    const categoryMatch =
      filteredCategory === "all" || skill.category === filteredCategory;
        ...
        {showConnections &&
          filteredSkills.map((skill, index) =>
            skill.connections?.map((connectionId, connIndex) => {
              ...
              if (!connectedSkill || !filteredSkills.includes(connectedSkill))
                return null;
              ...
              return (
                ...
                >
                  <line
                    ...
                    x2={`${
                      20 +
                      (skills.findIndex((s) => s.id === connectionId) % 5) * 20
                    }%`}
                    y2={`${
                      20 +
                      Math.floor(
                        skills.findIndex((s) => s.id === connectionId) / 5
                      ) *
                        25
                    }%`}
                    stroke="currentColor"
              ...
              <div className="absolute top-full left-1/2 transform -translate-x-1/2 mt-4 z-30">
                <Card variant="glass" className="w-64 p-4">
                  ...
                  <div className="space-y-2 text-xs">
                    ...
                    {skill.description && (
                      <p className="text-muted-foreground font-kabel mt-2">
                        {skill.description}
                      </p>
                    )}
                  </div>
          ...
          <div className="p-6">
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-muted-foreground font-kabel capitalize">
                  {skill.category}
                </p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span className="font-kabel text-muted-foreground">
                    Proficiency
                  </span>
                  <span className="font-basement text-foreground">
                    {skill.level}%
    ...
    <div className={cn("space-y-8", className)}>
              ...
              <div className="flex bg-white/20 rounded-lg p-1">
                {(["network", "grid", "tree"] as const).map((view) => (
                  <button
                    key={view}
                    onClick={() => {
                      /* View switching would be handled by parent */
                    }}
                    className={cn(
      ...
      <Card variant="glass" padding="lg">
        ...
        <CardDescription className="font-kabel">
          {variant === "network" &&
            "Network view showing skill relationships and proficiency levels"}
          {variant === "grid" &&
            "Grid view with detailed skill cards and progress indicators"}
          {variant === "tree" &&
            "Hierarchical view organized by technology categories"}
        </CardDescription>
        <CardContent>
          ...
          <div className="grid md:grid-cols-4 gap-4">
            <div className="text-center p-3 glass-card rounded-lg">
              <div className="w-12 h-12 bg-success-500/20 rounded-full mx-auto mb-2 flex items-center justify-center">
                <span className="text-xs font-basement text-success-500">
                  90+
                </span>
              </div>
              <div className="text-sm font-basement text-foreground">
                Expert
              </div>
              <div className="text-xs text-muted-foreground font-kabel">
            ...
            <div className="text-center p-3 glass-card rounded-lg">
              <div className="w-10 h-10 bg-primary-500/20 rounded-full mx-auto mb-2 flex items-center justify-center">
                <span className="text-xs font-basement text-primary-500">
                  70+
                </span>
              </div>
              <div className="text-sm font-basement text-foreground">
            ...
            <div className="text-center p-3 glass-card rounded-lg">
              <div className="w-8 h-8 bg-warning-500/20 rounded-full mx-auto mb-2 flex items-center justify-center">
                <span className="text-xs font-basement text-warning-500">
                  50+
                </span>
              </div>
              <div className="text-sm font-basement text-foreground">
            ...
            <div className="text-center p-3 glass-card rounded-lg">
              <div className="w-6 h-6 bg-neutral-500/20 rounded-full mx-auto mb-2 flex items-center justify-center">
                <span className="text-xs font-basement text-neutral-500">
                  30+
                </span>
              </div>
              <div className="text-sm font-basement text-foreground">
